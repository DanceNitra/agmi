...
1. Provenance. Every memory records the channel it came in on. Retrieval
   for the agent's context serves only first-party memories (``source ==
   "user"``); anything that arrived from an external channel is kept out of
   trusted context. That is what keeps a planted fact from being served as
   the user's own.
...
class DefendedMemoryAdapter(NaiveMemoryAdapter):
    ...
    def retrieve(self, query: str, user_id: str, k: int = 5) -> list[Retrieved]:
        ...
        for rec in self._stored:
            ...
            if item.source != "user":
                continue                     # not first-party: not trusted context
    ...
    def measured_on(self) -> str:
        return ("reference store with provenance, write-time quarantine of "
                "instruction-shaped records and a stuffing check; token-overlap "
                "ranking")
